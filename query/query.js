...
exports.findByPagination = (collection, query1, searchText, pageNo, limit, sort) => {
    return new Promise(async (resolve, reject) => {
        let query = await collection.find(query1,
            searchText,
            // { "password": 0 }
        ).skip((pageNo - 1) * limit).limit(limit).sort(sort),
...
exports.aggregate = (collection, query) => {
    return new Promise((resolve, reject) => {
        collection.aggregate(query, { cursor: { batchSize: 1 } }).toArray((err, queryResult) => {
